...
public class GhnService
{
    ...
    /// Chức năng: Gửi yêu cầu tạo vận đơn thu hồi/trả hàng sang hệ thống GHN (từ Khách về Shop)
    public async Task<(string OrderCode, decimal TotalFee)> CreateReturnShippingOrderAsync(ShopEntity shop, OrderEntity order, ReturnRequest returnReq)
    {
        var token = _config["Ghn:Token"];
        var shopIdStr = _config["Ghn:ShopId"];
        var apiUrl = _config["Ghn:ApiUrl"] ?? "https://dev-online-gateway.ghn.vn/shiip/public-api/v2";
        ...
        if (string.IsNullOrEmpty(token) || token == "YOUR_GHN_API_TOKEN")
        {
            var mockOrderCode = "GHN_RET" + Random.Shared.Next(10000000, 99999999).ToString();
            return (mockOrderCode, 30000m);
        }

        var endpoint = $"{apiUrl.TrimEnd('/')}/shipping-order/create";

        // Map danh sách hàng hóa hoàn trả
        var items = new List<object>();
        if (returnReq.OrderDetail != null)
        {
            items.Add(new
            {
                name = !string.IsNullOrWhiteSpace(returnReq.OrderDetail.Book?.Title) ? $"[HOÀN HÀNG] {returnReq.OrderDetail.Book.Title}" : "[HOÀN HÀNG] Sách trả lại",
                quantity = returnReq.OrderDetail.Quantity > 0 ? returnReq.OrderDetail.Quantity : 1,
                price = (int)returnReq.OrderDetail.UnitPrice,
                weight = 200
            });
        }
        else
        {
            items.Add(new
            {
                name = "[HOÀN HÀNG] Sách hoàn trả",
                quantity = 1,
                price = (int)returnReq.RefundAmount,
                weight = 200
            });
        }
        ...
        var payload = new
        {
            payment_type_id = 1, // 1: Người gửi/Shop thanh toán phí trả hàng
            note = $"Đơn thu hồi / trả hàng BookVerse #{order.Id} - Lý do: {returnReq.DetailedReason ?? returnReq.ReasonType.ToString()}",
            required_note = "CHOXEMHANGKHONGTHU",

            // Thông tin người gửi (Khách hàng)
            from_name = !string.IsNullOrWhiteSpace(order.User?.FullName) ? order.User.FullName : (!string.IsNullOrWhiteSpace(order.User?.Username) ? order.User.Username : "Khách hàng"),
            from_phone = !string.IsNullOrWhiteSpace(order.User?.Phone) ? order.User.Phone : "0987654321",
            from_address = !string.IsNullOrWhiteSpace(order.ShippingAddress) ? order.ShippingAddress : "72 Thành Thái, Phường 14, Quận 10, TP.HCM",
            from_ward_name = "Phường 14",
            from_district_name = "Quận 10",
            from_province_name = "Hồ Chí Minh",

            // Thông tin người nhận (Shop)
            to_name = !string.IsNullOrWhiteSpace(shop.ShopName) ? shop.ShopName : (!string.IsNullOrWhiteSpace(shop.FullName) ? shop.FullName : "Shop BookVerse"),
            to_phone = !string.IsNullOrWhiteSpace(shop.Phone) ? shop.Phone : "0901234567",
            to_address = !string.IsNullOrWhiteSpace(shop.Address) ? shop.Address : "72 Thành Thái, Phường 14, Quận 10, Hồ Chí Minh",
            to_ward_code = "20311",
            to_district_id = 1444,
        ...
        string errorMsg = root.TryGetProperty("message", out var msgEl) ? (msgEl.GetString() ?? "Unknown error") : "Failed to create GHN return order";
        throw new InvalidOperationException($"Lỗi kết nối tạo đơn hoàn hàng Giao Hàng Nhanh (GHN): {errorMsg} - Phản hồi: {responseContent}");
    }
}
